...
public class Validation {
  public static class Hostnane implements InputValidator<String> {
    @Override
    public String getMessage(String value) {
      return "invalid hostname or IPv4/IPv6 address";
    }
    ...
    public boolean isValid(String value) {
      try {
        return DomainValidator.getInstance(true).isValid(value)
            || InetAddressValidator.getInstance().isValidInet4Address(value)
            || InetAddressValidator.getInstance().isValidInet6Address(value);
      } catch (Exception e) {
        return false;
      }
    }
  ...
  public static class PortNumber implements InputValidator<String> {
    @Override
    public String getMessage(String value) {
      return "the port number must be greater than 1024 and less than 65536";
    }
  ...
  public static class NonEmpty implements InputValidator<String> {
  ...
  public static class Real implements InputValidator<String> {
    private final String name;
    ...
    @Override
    public String getMessage(String value) {
      return "the " + name + " must be a real number";
    }
  ...
  public static class PositiveReal implements InputValidator<String> {
    private final String name;

    public PositiveReal(String name) {
      this.name = name;
    }

    @Override
    public String getMessage(String value) {
      return "the " + name + " must be a positive real number";
    }
  ...
  public static class NonNegativeReal implements InputValidator<String> {
    private final String name;

    public NonNegativeReal(String name) {
      this.name = name;
    }

    @Override
    public String getMessage(String value) {
      return "the " + name + " must be a nonnegative real number";
    }
  ...
  public static class BoundedReal implements InputValidator<String> {
    private final String name;
    private final double min;
    private final double max;

    public BoundedReal(String name, double min, double max) {
      this.name = name;
      this.min = min;
      this.max = max;
    }
    ...
    @Override
    public String getMessage(String value) {
      return "the " + name + " must be in range [" + Double.toString(min) + ", " + Double.toString(max) + "]";
    }
  ...
  public static class RegexValidator implements Serializable {
     ...
     * @param regex The regular expression this validator will validate against
     * @param caseSensitive when <code>true</code> matching is <i>case sensitive</i>, otherwise matching is <i>case in-sensitive</i>
     */
   ...
   * @since Validator 1.4
   */
  public static class DomainValidator implements Serializable {
    ...
    /**
     * Returns true if the specified <code>String</code> matches any IANA-defined top-level domain. Leading dots are ignored if present. The search is case-insensitive.
...
//      "tp",                 // East Timor (Retired)
    ...
    /**
     * This field is used to detect whether the getInstance has been called. After this, the method updateTLDOverride is not allowed to be called.
     * This field does not need to be volatile since it is only accessed from synchronized methods.
     */
  ...
  public static class InetAddressValidator implements Serializable {
    ...
    private static final int IPV6_MAX_HEX_GROUPS = 8;           // Max number of hex groups (separated by :) in an IPV6 address
    private static final int IPV6_MAX_HEX_DIGITS_PER_GROUP = 4; // Max hex digits in each IPv6 group
